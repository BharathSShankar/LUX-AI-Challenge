...
class nnAgentTrainer(core.Actor):
    ...
    def choose_act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        actions, act_probs = self.controller.convert_output_to_actions(
            self.player, gameState, to_change, unit_actions_logits, fact_actions_logits, unit_actions_disc_params_R, unit_actions_disc_params_Rep, unit_actions_disc_params_N, obs_proc[
                "ACT_FACTS"], obs_proc["ACT_UNITS"]
        )
    ...
    def process_batch(self, to_change_old, global_states, img_states,
        ...
        to_change_new, unit_actions_logits, fact_actions_logits, \
            ...
            unit_actions_disc_params_N = self.actor.apply(
                actor_params, batch_global_states, batch_img_states, batch_fact_states, batch_unit_states)

        new_values = self.critic.apply(
            critic_params,  batch_global_states, batch_img_states)
        total_actor_loss = 0

        fact_loss = (jax.nn.log_softmax(fact_actions_logits, axis=2) -
                     jax.nn.log_softmax(batch_fact_probs, axis=2)).exp() * batch_fact_exist
        weighted_probs_fact = fact_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            fact_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
            jnp.minimum(weighted_probs_fact, weighted_probs_fact_clip).mean(
                axis=(0, 2)).sum()

        unit_loss = (jax.nn.log_softmax(unit_actions_logits, axis=2) -
                     jax.nn.log_softmax(batch_unit_probs, axis=2)).exp() * batch_unit_exist
        weighted_probs_fact = unit_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            unit_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
            jnp.minimum(weighted_probs_fact, weighted_probs_fact_clip).mean(
                axis=(0, 2)).sum()

        unit_loss = (jax.nn.log_softmax(unit_actions_disc_params_R,
                     axis=2) - jax.nn.log_softmax(batch_unit_probs_R, axis=2)).exp()
        weighted_probs_fact = unit_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            unit_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
        ...
        unit_loss = (jax.nn.log_softmax(unit_actions_disc_params_Rep,
                     axis=2) - jax.nn.log_softmax(batch_unit_probs_Rep, axis=2)).exp()
        weighted_probs_fact = unit_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            unit_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
        ...
        unit_loss = (jax.nn.log_softmax(unit_actions_disc_params_N,
                     axis=2) - jax.nn.log_softmax(batch_unit_probs_N, axis=2)).exp()
        weighted_probs_fact = unit_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            unit_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
        ...
        unit_loss = (jax.nn.log_softmax(to_change_new, axis=2) -
                     jax.nn.log_softmax(batch_to_change, axis=2)).exp()
        weighted_probs_fact = unit_loss * gae_val[batch]
        weighted_probs_fact_clip = jnp.clip(
            unit_loss, 1 - eps, 1 + eps) * gae_val[batch]
        total_actor_loss += - \
